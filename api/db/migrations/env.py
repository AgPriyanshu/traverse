...
from api.db import models  # noqa: F401  (registers every table on SQLModel.metadata)
...
# Use the DB URL from the app's settings instead of the placeholder in
...
def process_revision_directives(context, revision, directives) -> None:
    ...
    of Alembic's default random hash, so migration files sort and read like
    Django's (e.g. 0001_init_database.py).
    """
    script = directives[0]
    head_revision = ScriptDirectory.from_config(config).get_current_head()

    new_rev_id = 1 if head_revision is None else int(head_revision) + 1
